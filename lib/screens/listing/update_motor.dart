...
class MotorForm extends StatefulWidget {
  ...
  const MotorForm({this.listingData, this.isEditing = false, Key? key})
      : super(key: key);
...
class _MotorForm extends State<MotorForm> {
  ...
  void _initializeEditingListing() {
    ...
    final makeExists =
        motormake.any((motor) => motor.brand.contains(data['make']));
    selectedMake = makeExists ? data['make'] : 'Other';

    // If it's a custom make, set the custom make controller
    if (!makeExists) {
      customMakeController.text = data['make'] ?? '';
    }

    // Set the model value if it exists (optional)
    if (data['model'] != null) {
